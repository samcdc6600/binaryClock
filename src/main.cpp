...
{ /* Number of args to be passed when run without a config file. The argument's
     shoule be as follows:
     X coordinate, Y coordinate, alpha value,
     background red value,	background green value,
     background blue value,
     text red value,		text green value,
     text blue value,
     positive bit red value,	positive bit green value,
  ...
  if(argc != cliCoordsArgcNum)
	...
	{ /* Our coordinates are comming from configPath (and their may be more
	  ...
	  if(getConfigurableParameters(configPath.str().c_str(), coords))
		...
		{		/* Start clocks after first one specified in
				   config file :) */
      ...
      else
	{
	  std::cerr<<"Usage (with arguments): \""<<argv[0]<<" x y\", where x & "
	    ...
	    " arguments are passed the x & y coordinates are read in from the "
  ...
  constexpr int cliCoordXIndex {1}, cliCoordYIndex {2};
  ...
  try
    { // Stoi may throw invalid_argument exception or out_of_range exception
      ...
      if(init(stoi(x), stoi(y), color, usingConfig, configPath.str().c_str(),
	      con, winHeight))
	{
	...
	"correct format is \""<<argv[0]<<" x y\" (where x & y are the "
	"coordinates of the top left corner of the window and the ranges of x "
	"and y are both [0, "<<((long(2)<<((sizeof(int) * 8) -1)) /2) -1
	       <<"].)\n";
    }
...
/*
  ...
  extensions
*/
bool init(const int winX, const int winY, Color & color,
	  const bool usingConfig, const char * configPath, context & con,
	  const int & winHeight)
{
  using namespace tunables;
  bool ret {false};
  ...
  XMatchVisualInfo(con.display, DefaultScreen(con.display), 32, TrueColor,
		   &con.vinfo);
  con.attribs.colormap = XCreateColormap(con.display,
					 DefaultRootWindow(con.display),
					 con.vinfo.visual, AllocNone);
...
{
  size_t iter {};
  for(int magnitude {1}; time[iter] != ':' && time[iter] != '\0' && iter <
	time.size(); ++iter, magnitude *= 10)
		     ...
		     const int winHeight)
{
  ...
  for(unsigned long rowIter {bits.size()}; rowIter > 0; --rowIter)
	...
	XFillRectangle(con.display, con.window, con.gc,
		       (hGap*(column+1)) + (width*column), winHeight -
