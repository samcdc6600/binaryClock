...
struct context
{
  ...
  std::vector<XColor *> xColors {&cyan, &purple, &blue, &green, &yellow,
				 &orange, &red, &darkRed};
  GC gc;
};

constexpr unsigned int winWidth {91}, winBoarder_width {};
// Not all of the variables on this line and the next two need to be global
constexpr int numBitsHour {5};	// because they arn't used in more then.
// One function. However since we are using some of them to calculate the window
constexpr int numBitsMinSec {6}; // height and they are all.
// Constants it was deemed acceptible make them all global since they are all
constexpr int hGap {10}, vGap {10}, width {17}, height {17}; // related.
constexpr int textHeight {10};


bool getConfigurableParameters(const char * configPath,
			       std::vector<int> & coords);
void mainLoop(const int winX, const int winY, const bool usingConfig,
	      const char * configPath);
int calcWinHeight();
bool init(const int winX, const int winY, const bool usingConfig,
	  const char * configPath, context & con, int & winHeight);
...
inline void extractTimeFields(const std::string time, int & hours,
			      int & minutes, int & seconds);
inline void strNumPosToInt(const std::string & time, int & number,
			   const int magnitude, const int iter);
inline void getBits(const int numBits, const int number,
		    std::vector<bool> & ret);
inline void drawBits(context & con, const std::vector<bool> & bits,
		     const int column, const int winHeight);
...
/* Feature to add! have program take extra argument when it run's it's self so
   ...
   (in init())*/
// -----------------------------------------------------------------------------
/* Change string parsing for cliCoordX and cliCoordY so that no trailing
   characters are allowed after the numbers */
// -----------------------------------------------------------------------------
/* Add colour adjustment feature */
// -----------------------------------------------------------------------------
/* Add transperancy feature */
// -----------------------------------------------------------------------------
/* FEATURES TO ADD END! -- FEATURES TO ADD END! -- FEATURES TO ADD END! */

int main(int argc, char * argv[])
{		    // Number of args passed when run with coordinates
  constexpr int cliCoordsArgcNum {3};
  ...
  if(argc == cliCoordsArgcNum)
      ...
      catch(const std::invalid_argument & e)
	{
	  std::cerr<<"Error ("<<e.what()<<"): x and or y are not numbers!\nThe "
	    "correct format is \""<<argv[0]<<" x y\" (where x & y are the "
      ...
      catch(const std::out_of_range & e)
	{
	  std::cerr<<"Error ("<<e.what()<<"): x and or y are out of range!\nThe"
	    "correct format is \""<<argv[0]<<" x y\" (where x & y are the "
	    "coordinates of the top left corner of the window and the ranges of"
	    "x and y are both [0, "
	...
	{ /* Our coordinates are comming from configPath (and their may be more
	  ...
	  if(getConfigurableParameters(configPath.str().c_str(), coords))
		...
		{ // Start clocks after first one specified in config file :)
		  ...
		  cmdFull<<cmdStart.str()<<coords[iter]<<' '<<coords[iter +1]
			 <<background;
		  system(cmdFull.str().c_str());
		}
	      // Start clock associated with this process
	      mainLoop(coords[firstX], coords[firstY], usingConfig,
		       configPath.str().c_str());
	    }
	}
      else
	{
	  std::cerr<<"Usage (with arguments): \""<<argv[0]<<" x y\", where x & "
	    "y are the coordinates of the top left corner of the window.\nIf "
	    "no arguments are passed the x & y coordinates are read in from the"
	    "configuration file \""<<configPath.str()<<"\" (note that you can "
	    "specify multiple coordinates in the config file, in which case a "
	    "new instance of the program will run (with the corresponding pair "
	    "of coordinates passed to it as it's arguments) for every pair of "
			       ...
			       std::vector<int> & coords)
{
  ...
  if(in.is_open())
    {
      constexpr char subCoordSpacer {','}, interCoordSpacer {':'},
					     endChar {';'};
      char skip {}, end {};
      int x {}, y {};
      bool more {true};
	...
	"\nUsage:\t\"x1,y1:x2,y2: ... ;\".\nWhere x & y are the "
	"coordinates of the top left corner of the window/s, '"
	      <<subCoordSpacer<<"' act's as the sub-field seperator, '"
	      <<interCoordSpacer<<"' acts as the inter-field seperator and the "
	"fact that there can be effectively any number of coordinates (but "
	"there must be at least one) is denoted by the ellipsis after the "
	"second field (\"x2,y2\"). Finally the last field must be followed by '"
	      <<endChar<<"'.\n";
      ...
      while(more)
	{
	  ...
	  if(skip != subCoordSpacer)
	    {
	      std::cerr<<"Error: integer or '"<<subCoordSpacer<<"' missing! "
		       <<errorMsg.str();
	      ret = false;
	      more = false;
	    }
		...
		{	// We should be at the end and have an endChar
		  if(end == endChar)
			...
			{	/* I don't think this point will ever be
				   reached. However I am not going to remove it
				   right now. I plan on improving the code
				   within this while loop in the future. */
		    ...
		    {
		      std::cerr<<"Error: integer, '"<<subCoordSpacer<<"' or '"
			       <<endChar<<"' expected! "<<errorMsg.str();
		      ret = false;
		      more = false;
		    }
		}
	    }
	  /* Reset in case there is not enough characters to read in on the next
	     iteration */
    ...
    {
      std::cerr<<"Unable to open file \""<<configPath<<"\". $HOME environment "
	"variable may not be set or file may not exist!\n";
      ret = false;
    }
...
int calcWinHeight()
{ /* textHeight and vGap are multiplied by two the same multiplication is done
     in xdraw string (pleas clean this up and make all the code nice :) .) */
  return (vGap * (numBitsMinSec +1)) + (height * numBitsMinSec) + textHeight*2
    + vGap*2;
}
...
/*
  ...
  "Display" in xlib / x11 protocol terminology is one single connection between
  client and X server.

  "Screen" is actual screen, but things get more complicated here. Each screen
  has its own root window ( and some more associated properties physical
  width/heights, DPI etc ). Because every window on the screen is child of that
  root window, you can't just move window from one screen to another (all child
  windows under X11 always clipped by parent). This is one of the reason
  multiple "screens" as in your question almost never used most people have
  multiple monitors configured to be part of one X11 screen using Xinerama/RANDR
  extensions
*/
bool init(const int winX, const int winY, const bool usingConfig,
	  const char * configPath, context & con, int & winHeight)
{
  ...
  /* To do subtract width of window from WidthOfScreen(s) (Also make sure that
  ...
  if(!(winX >= 0 && winX <= WidthOfScreen(s) - winWidth)) 
      ...
      else
	{
	  ...
	  con.window = XCreateWindow( con.display, RootWindow(con.display, 0),
				      ...
				      winBoarder_width, CopyFromParent,
	  ...
	  std::vector<std::string> colors {"Cyan", "Purple", "Blue", "Green",
					   "Yellow", "Orange", "Red",
					   "Dark Red"};
	  ...
	  for(int iter {}; iter < colors.size(); ++iter)
	    {
	      rc = XAllocNamedColor(con.display, con.cmap, colors[iter].c_str(),
				    con.xColors[iter], con.xColors[iter]);
	      if(rc == 0)
		{
		  std::cerr<<"XAllocNamedColor - failed to allocated '"
			   <<colors[iter].c_str()<<"' color.\n";
		  exit(1);
		}
	    }
	}
...
inline void draw(context & con, const time_t time, const int winHeight)
{			 //clear the window before we draw to it again
  XClearWindow(con.display, con.window);
  ...
  XDrawString(con.display, con.window, con.gc, hGap , textHeight + vGap,
	      dateStart.str().c_str(), dateStart.str().size());
  XDrawString(con.display, con.window, con.gc, hGap, textHeight*2 + vGap*2,
	      dateEnd.str().c_str(), dateEnd.str().size());
}
			      ...
			      int & minutes, int & seconds)
{
  int iter {};
  for(int magnitude {1};
      time[iter] != ':' && time[iter] != '\0' && iter < time.size();
      ++iter, magnitude *= 10)
		     ...
		     const int column, const int winHeight)
{
  for(unsigned long rowIter {bits.size()}; rowIter > 0; --rowIter)
	...
	XFillRectangle(con.display, con.window, con.gc,
		       (hGap*(column+1)) + (width*column),
		       winHeight - ((vGap*rowIter) + (height*rowIter)), width,
		       height);
      else
	{
	    ...
	    XFillRectangle(con.display, con.window, con.gc,
			   (hGap*(column+1)) + (width*column),
			   winHeight - ((vGap*rowIter) + (height*rowIter)),
			   width, height);
	}
    }
}
